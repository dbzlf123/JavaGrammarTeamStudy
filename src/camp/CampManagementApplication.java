    ...
    private static void displayStudentView() {
        ...
        while (flag) {
            ...
            switch (input) {
                ...
                case 3 -> inquireStudentInfo(); // 수강생 정보 조회 //이름바꿀 필요있음.
...
//이름이 같을 경우도 고려해야함
    ...
    public static void inquireStudentInfo() {
        //학생 id 입력으로 조회
        //해당학생 id for 문 studentStore.size 돌면서 맞는 id
        // 찾고 해당 위치 아이디 이름 상태 과목 받아오기 이런식으로 가면 될듯
        // for 문 size 돌리고 if 문으로 같은지 검색하고 안에 해당 학생 정보 내고
        System.out.println("\n수강생 목록을 조회합니다...");
        ...
        for (int i = 0; i < studentStore.size(); i++) {
            if(studentId.equals(studentStore.get(i).getStudentId())) { //입력받은 id가 스튜던트 스토어에 같은값이 있다면 진행
                System.out.println("고유번호: "); //과목 이름 받아오는 메서드가 세로로 나열되서
                //비슷하게 맞추기 위한 세로 정렬
